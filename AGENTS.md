...
- **Naming** (README → *Bundling → Naming*): the scripts are Python but named like
  ...
  `{subject}-graph` (e.g. `gdvb-sockets-graph`); their bundles are `{subject}-graph-scope`
  in `dist/`; the viewer is `gdvb-render`.
...
Not every script is a generator: **`gdvb-scrub`** is a *filter* (reads a model JSON on
...
verb-noun (like `gdvb-render`); generators are `{subject}-graph`.

**`gdvb-lsp-graph`** is the other odd one out: a shared LSP-protocol *client*, not a generator
...
flags, so gdvb-lsp-graph stays server-agnostic by not touching either. `gdvb-clangd-lsp-graph` /
`gdvb-pyright-lsp-graph` are the actual generators: each spawns its server over stdio,
bridges that to a fresh Unix domain socket, and runs `gdvb-lsp-graph --connect` pointed at
...
gdvb-clangd-lsp-graph` produces a working `dist/gdvb-clangd-lsp-graph-scope`) — verified by
...
separate `gdvb-lsp-graph` reachable at runtime (`PATH`, or copied alongside in `dist/`),
since they orchestrate it as its own process rather than importing it. Adding a new
language/server means a new small wrapper script in this same shape; `gdvb-lsp-graph`
...
editing `gdvb-render`'s embedded `<script>` (especially via `sed`/scripted
